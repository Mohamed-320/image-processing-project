...
%Read in the image
image = imread('images/Picture 1.jpg');
...
threshold = bwareaopen(thresh,100);
...
%function is created to find the connected components and
%take out any pixel less than 1000 pixels as it is a noise not a fruit
...
for i = 1:CC.NumObjects
    %extract an image with individual fruit neglecting others
    temp = zeros(row,col);
    temp(CC.PixelIdxList{i}) = 1;
    figure, imshow(temp)
    title('Indiviual Fruit that was extracted out')
    
    %Use regionprops functions to get the bounding box to take 
    %the image and other feature of the fruit
    ...
    %get short to longer width and height vector to reduce orientation problem
    ...
    %Threshold the (neglected) unneccesarry parts in the RGB to white
    ...
    [ color ] = findFeat(rgbOut);
    Xnew = [stats.Eccentricity, longer/1000, shorter/1000, color/3];
    vector(i,:) = Xnew;
   
    % return a matrix, indicating the likelihood that a label comes 
